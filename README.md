...
- 主实验固定官方 OpenClaw / 官方 agent，不对 agent 做任何更改。
- 本项目主线是模型 post-training，只替换模型 checkpoint。

正式协议见 `docs/paper/evaluation-protocol.md`，评测配置模板见 `configs/eval/`。

## 快速开始

当前阶段还没有接入真实 OpenClaw 和模型服务，但仓库已经可以运行基础检查。

本机非敏感运行配置可以放在 `.env.local`，例如模型 endpoint、provider、model name 和 runs 目录。API key 仍然只放在你的 shell 环境变量里，不写进仓库文件。
...
第一次在新机器上准备本地虚拟环境时，可以运行：

```bash
make bootstrap
```

`doctor` 里关于 `ARE2TRAIN_OPENCLAW_HOME`、`ARE2TRAIN_MODEL_ENDPOINT`、`ARE2TRAIN_RUNS_DIR` 的 warning 是当前阶段的预期结果。第 02 章只检查仓库基础环境，第 04 章才会接入真实 baseline runner。
...
第一阶段目标是让 Qwen3-14B 在不改官方 OpenClaw / 官方 agent 的前提下，在 held-out ARE/Gaia2-style 任务上获得可衡量提升，同时降低工具调用错误率，并改善每次成功任务的成本。
...
1. **Baseline 复现**
   - 用官方 OpenClaw 运行 Qwen3-14B base checkpoint，跑一小批 ARE/Gaia2-compatible 任务。
...
6. **RLVR / GRPO**
   - 使用 verifier rewards。
   - 在不改官方 agent 的前提下，对比 base、SFT、preference、RLVR checkpoints。
