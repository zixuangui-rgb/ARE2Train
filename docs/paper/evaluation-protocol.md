...
| 开发集 | 自建 validation scenarios | 调试 runner、prompt、checkpoint、训练参数 | 不直接训练 |
| 最终测试集 | 官方 ARE/Gaia2 held-out tasks | 最终评测和论文主结果 | 不可以 |

官方 ARE/Gaia2 held-out tasks 只能用于最终评测。不得用于训练、数据生成、checkpoint 选择、prompt 调参或 reward 设计。

## benchmark leakage 防护规则

- 不把官方 held-out tasks 加入训练集。
- 不根据官方 held-out 错误反复修改 prompt。
...
所有 baseline 必须使用同一套评测任务和同一套结果记录格式。
...
- 不使用 RLVR。
- 不使用 reflection memory。
- 不使用 action-level reward，只用最终 pass/fail。
- 不使用改进后的 agent scaffold。
...
- 评测前模型、prompt、checkpoint 和配置已经冻结。
...
- 主要 claim：在官方 ARE/Gaia2 held-out tasks 上获得可验证提升，并分析提升来自哪些模块。
