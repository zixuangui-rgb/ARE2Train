...
如果没有这一步，后面很容易出现一个问题：模型分数变高了，但别人不知道这个分数是不是真的可信。比如你可能不小心用测试题调了 prompt，或者不同模型用了不同任务集合，最后结果看起来提升了，但其实不能说明模型真的变强。
...
- **真的变强**：不能只是记住题目，也不能只是换了 prompt 后刚好撞对。
...
更隐蔽的泄漏是：虽然没有直接训练测试题，但你反复看官方测试集错误，然后不断改 prompt、改数据生成器、改 reward。这样也会慢慢针对测试集优化，最后结果同样不够可信。
...
你会调整数据过滤规则、prompt、LoRA 参数、RLVR 参数、agent scaffold。如果每次调整都去看官方 held-out 分数，就等于反复偷看正式考试。

所以我们用自建 validation scenarios 做内部调试。它们不代表最终成绩，只帮助我们决定下一步怎么改。

### 什么是 agent scaffold

agent scaffold 可以理解成“包在模型外面的那层工作流程”。

模型本身只负责根据输入生成输出。可是 agent 任务通常不只是回答一句话，它还需要调用工具、读取工具返回、决定下一步动作、处理错误、判断什么时候停止。

把这些步骤串起来的外层流程，就是 agent scaffold。
...
用户说：帮我把会议改到周五
模型决定：先查日历
scaffold 执行：调用 calendar.search
模型看到结果后决定：更新会议时间
scaffold 执行：调用 calendar.update
verifier 检查：会议是否真的改对
```

所以我们必须区分两件事：

- agent scaffold 变好了：比如 prompt 更清楚、工具调用格式更稳定、错误重试更完善。
- 模型训练变好了：比如 Qwen3-14B 经过 SFT 或 RLVR 后，本身更会完成任务。

如果不区分这两件事，分数提升了也说不清原因。
...
| 开发集 | 自建 validation scenarios | 调 prompt、调 runner、选 checkpoint | 不直接训练 |
| 最终测试集 | 官方 ARE/Gaia2 held-out tasks | 最终评测和论文主结果 | 绝对不能训练 |

官方 ARE/Gaia2 held-out tasks 只能用于最终评测。不能用于训练，不能用于选择 checkpoint，也不能用于反复调 prompt。
...
- 原始 Qwen3-14B。
- Qwen3-14B + 原始 agent scaffold。
- Qwen3-14B + 改进后的 agent scaffold。
- Qwen3-14B + SFT。
- Qwen3-14B + SFT + preference tuning。
- Qwen3-14B + SFT + preference tuning + RLVR。
- teacher model 参考线。

这里最重要的是区分两种提升：

- 只是 agent scaffold 变好了。
- 模型本身经过训练后变好了。
...
- 保存完整 trace。
- 记录模型版本、prompt 版本、scenario 版本和 checkpoint 版本。
- 记录运行配置和随机种子。
- 报告失败类型，不只报告成功率。
- 不根据官方 held-out 错误反复调 prompt 或训练数据。
...
- 过早看官方 held-out 结果，然后根据错误反复调 prompt。
- 只报告总体 pass rate，不分析失败类型。
- 把 agent scaffold 的提升，说成模型训练的提升。
- 不记录 prompt、模型、数据和 checkpoint 版本。
- 用不同任务集合比较不同模型。
