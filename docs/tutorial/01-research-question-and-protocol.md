...
- agent scaffold 是外面的执行流程。
- 模型 checkpoint 是我们真正要训练和比较的对象。

本项目的主线很明确：**固定官方 OpenClaw / 官方 agent scaffold，只调整模型 checkpoint**。

也就是说，我们不是要证明自己写了一个更好的 agent 外壳，而是要证明同一个官方 agent 外壳下，模型经过 SFT、preference tuning、RLVR 后真的变强。
